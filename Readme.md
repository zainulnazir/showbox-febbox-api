...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/elsayed85/show_feb_box_api.git
    cd show_feb_box_api
    ```
...
3.  **Setup the Cloudflare Bypass Server**:
    The bypass server is located in `cf-bypass/` and runs on Python.
    ```bash
    cd cf-bypass
...
4.  **Configuration**:
    ...
    cp .env.example .env
    ```
    *Edit `.env` to add your optional configuration.*
...
*   **[CloudflareBypassForScraping](https://github.com/sarperavci/CloudflareBypassForScraping)** by **sarperavci**:
    *   This project is the backbone of our Cloudflare bypass mechanism. We have integrated it directly into our `cf-bypass/` directory to enable seamless automated link extraction. A huge thanks to sarperavci for maintaining this excellent tool.
...
The `cf-bypass` component is also licensed under MIT by its original author (sarperavci).
